...
extern unsigned _int64 wsym[norw]; // 每一个保留字对应的symbol类型表
extern unsigned _int64 ssym[256];  // 每一个符号对应的symbol类型表
extern unsigned _int64 sym;        // 读到的最后一个token的类型
extern long num;                 // 读到的最后一个数字

void getch();                    // 读取单个字符
void getsym();                   // 读取一个token
void test(unsigned _int64, unsigned _int64, long);
                                 // 测试当前token是否合法
...
extern unsigned _int64 declbegsys; // decl开始符号集合
extern unsigned _int64 statbegsys; // stmt开始符号集合
extern unsigned _int64 simpexpbegsys;// simpexp开始符号集合
extern unsigned _int64 termbegsys; // term开始符号集合
extern unsigned _int64 facbegsys;  // factor开始符号集合
extern long exitlist[elsize];    // while中的exit地址表
extern long elx;                 // exitlist指针

void block(unsigned _int64);
void constdeclaration();
void vardeclaration();
void statement(unsigned _int64);
void expression(unsigned _int64);
void simpexpression(unsigned _int64);
void term(unsigned _int64);
void factor(unsigned _int64);
